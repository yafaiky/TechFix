      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredServices.map((s) => {
            ...
            const isFinal = FINAL_STATUSES.includes(s.serviceStatus);

            return (
              <div
                key={s.id}
                className={`p-4 border rounded-lg shadow flex flex-col justify-between transition ${
                  isFinal ? "bg-gray-100 opacity-90" : "bg-white"
                }`}
              >
                <div>
